...
class Settings(BaseSettings):
    ...
    web_search_endpoint: str = "https://api.search.brave.com/res/v1/web/search"
    web_search_api_key: str | None = None
    max_repo_files: int = 2000
    max_github_issues: int = 1000
    max_agent_rewrites: int = 2
    token_budget: int = 6000
    citation_repair_enabled: bool = True
    citation_support_threshold: float = 0.18
    input_cost_per_million_tokens: float = 0
    output_cost_per_million_tokens: float = 0
    worker_poll_seconds: float = 0.25
    worker_heartbeat_seconds: float = 2
    worker_stale_seconds: float = 30
    worker_fault_pause_after_claim_seconds: float = 0
